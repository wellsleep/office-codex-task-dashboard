...
设计与实现细节见 [设计 spec](DESIGN.md)，包括技术栈、数据契约、发布心跳与故障恢复。

## 页面与更新

- 最近 24 小时结果、各任务最新状态、异常队列、近七天趋势。
- 按任务、结果和时间范围查询历史；默认七天，可查看九十天或自定义日期。
- 有变化即推送；没有变化每两小时发布一次采集心跳。GitHub 构建与缓存会产生额外延迟。
- 超过三小时未更新显示同步过期。Mac 休眠期间不能采集，恢复后补采。
...
测试覆盖动态新任务、ID 重用、重复发现去重、原对话标记、字段变化、来源缺失、源数据只读、状态分类、脱敏和每两小时心跳。页面交互与手机布局另用浏览器验收。
